...
def combat(body, infection, boost):

    body_army = {idx: Unit(x, boost) for idx, x in enumerate(body)}
    infection_army = {idx: Unit(x, 0) for idx, x in enumerate(infection)}
    ...
    while body_army and infection_army and (not stalemate):
        ...
        for team, idx, _ in initiative:
            ...
            # attack happens
            n_died = defender.get_damage(attacker.effective_power, attacker.attack)
            if n_died > 0:
                stalemate = False

    b = sum(u.units for u in body_army.values())
    ii = sum(u.units for u in infection_army.values())
...
def solve(body, infection):
    ...
    solution = combat(body, infection, boost)
